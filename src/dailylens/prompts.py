...
{{"description": "사용자가 하고 있는 활동을 1-2문장으로 설명", "category": "코딩/문서작성/이메일/웹브라우징/미팅/디자인/커뮤니케이션/기타 중 하나"}}""",
        "ocr_header": """
### 화면에서 추출된 텍스트 (OCR)
아래는 스크린샷에서 OCR로 추출한 텍스트입니다. 이 텍스트를 참고하여 사용자의 활동을 더 정확하게 파악해주세요.
```
{ocr_text}
```""",
        "context_header": """
...
{{"description": "Describe the user's activity in 1-2 sentences", "category": "one of: coding/writing/email/browsing/meeting/design/communication/other"}}""",
        "ocr_header": """
### Text extracted from screen (OCR)
Below is text extracted from the screenshot via OCR. Use this to more accurately identify what the user is doing.
```
{ocr_text}
```""",
        "context_header": """
...
{{"description": "ユーザーの活動を1-2文で説明", "category": "コーディング/文書作成/メール/ウェブ閲覧/ミーティング/デザイン/コミュニケーション/その他 のいずれか"}}""",
        "ocr_header": """
### 画面から抽出されたテキスト（OCR）
以下はスクリーンショットからOCRで抽出したテキストです。ユーザーの活動をより正確に把握するために参考にしてください。
```
{ocr_text}
```""",
        "context_header": """
...
{{"description": "用1-2句话描述用户的活动", "category": "编程/文档/邮件/浏览网页/会议/设计/沟通/其他 之一"}}""",
        "ocr_header": """
### 从屏幕提取的文字（OCR）
以下是通过OCR从截图中提取的文字。请参考这些文字更准确地判断用户的活动。
```
{ocr_text}
```""",
        "context_header": """
